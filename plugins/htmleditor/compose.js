...
function document.onreadystatechange() {
  if (YInitialized || typeof(Composition) == 'undefined') return;
  YInitialized = true;
  ...
  //Composition.document.write("<html><BODY onload='parent.loadcontent()'></body></html>");
...
//    PrevTB.style.width = LastWid;
	if (jojowidth == 0)
	{
		jojowidth = PrevTB.style.width;
	}
	else
	{
		PrevTB.style.width = jojowidth;
	}
...
function addImage() {
  ...
  var isA = getEl("A",Composition.document.selection.createRange().parentElement());
  var str=prompt("Enter image path (e.g. image.gif):", isA ? isA.href : "");
  
  Composition.document.execCommand("InsertImage","",str)
  Composition.focus();
}
...
function foreColor() {
  if (! validateMode()) return;
  var arr = showModalDialog("ColorSelect.html?3", "", "font-family:Verdana; font-size:12; dialogWidth:30em; dialogHeight:35em");
